...
| `repo-usage` | **Space report:** per-package tag count + total size, biggest first, rendered into the run summary. The coarse view — which *package* is heavy, before `list-rpms` shows individual versions. Read-only. | none |
